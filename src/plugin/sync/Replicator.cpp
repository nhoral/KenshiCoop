...
Replicator::Replicator()
    : leaderOnly_(true), activeFrames_(0), zeroWhileActive_(0), maxStep_(0.0f),
...
void Replicator::publishOwned(GameWorld* gw, NetLink& net, u32 ownerId) {
    EntityState buf[ENTITY_BATCH_MAX];
    unsigned int n = engine::captureSquad(gw, leaderOnly_, buf, ENTITY_BATCH_MAX);
...
void Replicator::applyTargets(GameWorld* gw) {
    ...
    for (std::map<Key, Driven>::iterator it = targets_.begin(); it != targets_.end(); ++it) {
        ...
        if (hostMoving && haveActual && haveNewest && gapNewest > SNAP_DIST) {
            ...
            // Source at rest: settle once (clean halt+teleport), then only
        ...
        if (hostMoving && haveActual && d.haveActual) {
