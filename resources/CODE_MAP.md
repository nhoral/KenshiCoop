...
| `EngineEntity.cpp` | EntityState capture (`captureOne`/`captureSquad`/`captureNpcs`), hand resolve (`resolve`/`resolveCharByHand`/`readObjectHand`), motion apply/orders, NPC suppression, interest centers (protocol 43: up to 4 anchors = 2 tab leaders + local camera + peer camera hint, deduped ~100u, `KENSHICOOP_CAM_INTEREST` default ON; `cameraCenter` reads `gw->player->camera` via `CameraClass::getCenter` with throttled `[cam] center=` log; `setLocalCamAnchor`/`setPeerCamHint`/`setCamInterest` anchor stores; `interestAnchors` SEH wrapper for the sync layer), debug marker HUD, seat/bed/cage/machine template + work-fixture finders | `g_npcQuery`, `g_dataScratch`, hand/locomotion `g_*Fn` |
...
| `EngineSpawnCombat.cpp` | Protocol 21 runtime-spawn proxies (`spawnProxyNpc`/`describeCharacter`), scenario scene setup (seat/bed/cage/machine/duel/down), combat reads + melee orders + `forceAttack` (`Character::attackTarget`, the AI's own commit-an-attack entry - bypasses the local re-decision that drops goal/order attacks on a player-owned non-hostile target), limb-loss/revive/bandage medical primitives | `g_npcQuery`, `g_dataScratch`, spawn/combat/medical `g_*Fn`; owns duel/down anchor statics |
...
| `[jail] STATE` | ReplicatorPublish.cpp `publishOwned` (`side=own`) + ReplicatorDrive.cpp `applyTargets` (`side=drv`), gated by `KENSHICOOP_JAIL_PROBE` (off; ~4/s per captive body; run_test arms it + `TASK_SPIKE` for `jail_probe`/`jail_soak`; manual `-JailProbe`) | spike 57 - correlated captive-state trace (`kind`/`chained`/`slaveOwner`/`isSlave`/`task`/`streamKind`/`localKind`/`pos`) for owned PC vs host driven copy; pins the jail "put to work" twitch (host self-heal re-cages a peer-owned copy its guard dragged out). `isSlave` via `readSlaveState` (`Character::isSlave`) | diag |
| `[jail] OBSERVE` | ReplicatorDrive.cpp `applyTargets`, gated by `KENSHICOOP_JAIL_OBSERVE` (off; jail_probe only, or manual `-JailProbe -JailObserve`) | spike 57 phase A - runs a peer-owned captive UNOPPOSED (no drive/suspend/self-heal) and logs its full trajectory (`localKind`/`streamKind`/`inWhat`/`slave`/`task`/`pos`/`step`) to classify the guard put-to-work intent (fixture-relocate vs job walk-round). WARNING: disabling the self-heal makes captives exit furniture + walk off on the observing side (diagnostic divergence, not a real desync); NOT for parity A-B | diag |
| `[jail] SNAP` | ReplicatorDrive.cpp `applyTargets` furniture heal, gated by `KENSHICOOP_JAIL_PROBE` (off) | spike 58 - quantifies each captive re-seat: `divergence=` (how far the driven copy drifted from the owner's streamed pos = the visible teleport magnitude) and `localStep=` (drift since last tick while nominally seated; >0 => the driven copy's own local AI is walking it - the "exit cage to run" half of the oscillation) | diag |
...
| `ReplicatorPublish.cpp` | `publishOwned` (owned squad + near band + 2 Hz mid band + combat-intent capture incl. canonical-hand `CAP xlate`), `publishNpcCensus` (mid-band priority = distance to nearest interest ANCHOR), `syncCamHint` (protocol 43: both sides publish the local camera to `engine::setLocalCamAnchor`; join sends `PKT_CAM_HINT` ~1 Hz; host stores `peerCam_`+`peerCamMs_` and feeds a fresh (<3 s) hint to `engine::setPeerCamHint`; `[cam] hint recv` log) |
| `ReplicatorDrive.cpp` | `applyTargets` (per-frame drive: walk/park/combat/carry/furniture/stealth + interp consumption + snap gates; combat reissue escalates goal-path -> order-path (`seatbrk=1`) -> `engine::forceAttack` (`forced=1`) when the local AI keeps dropping the attack), `applyRest`, `sweepCarries`, `logHardSnap` |
| `ReplicatorAuthority.cpp` | `applyNpcCensus`, `enforceHostAuthority` (suppress/park/cull + far-mint requests), `parkDivergedCopy` (returns drift; breaks a local furniture anchor first, never re-chains - kind-3 re-entry snapped the body back to the local slaveOwner; 1 s park cooldown while FROZEN, 5 s otherwise), `censusFreezeDivergedAi` (census-band AI freeze, `KENSHICOOP_CENSUS_FREEZE_AI` default ON: suspends the local AI of a census-band body that drifts past `censusParkDist_` so a captive/working slave can't flee/aggro the join's guards; divergence-gated + 20 s hold; per-tick `engine::haltMovement` kills the pre-freeze destination; `[census] FREEZE` log), debug markers |
...
| `[speeddbg]` (setGameSpeed/userPause/togglePause/poll) | EngineInternal + EngineCharState, gated by `KENSHICOOP_DEBUG_SPEED=1` (Phase 5 spike; run_test sets it for `speed*` scenarios) | diagnostic only - no oracle; logs setter+args+guard+combat+btn/vote to attribute engine-forced caps vs user clicks |
| `[shackledbg]` (per-body chained/lock trace) | EngineCharState `shackleDbgTick` (called from Plugin tick), gated by `KENSHICOOP_DEBUG_SHACKLE=1` (Phase 6 6a spike; run_test sets it for `shackle*` scenarios) | diagnostic only - logs hand/chained/shackleItem/lock/owner ~1 Hz for nearby world NPCs on both clients; pairs with `SCENARIO SHACKLE` / Test-ShackleProbe to characterize the peer-unlock desync |
...
| `SPAWN`, `WNPC`, `WORLD` (census rows) | Npc, Movement | Get-SpawnHands/Test-Spawn* (Npc.ps1), Get-WnpcRows/Test-TravelParity/Test-WorldParity (Motion.ps1; WorldParity = tiered PC/near/census parity on the `world_parity` camp scenario - fight-class task pairs (TASK_COMBAT_* synthetics + native attack/chase/first-aid TaskTypes) are excluded from the gated near-task ratio and reported as an advisory `combat=` ratio) |
