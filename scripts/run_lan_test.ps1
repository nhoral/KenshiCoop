...
$runArgs = [pscustomobject]@{
    ...
    env          = [pscustomobject]@{
        ...
        KENSHICOOP_INV_DUMP           = $(if ($Scenario -eq "inv_wpnseq" -or $Scenario -eq "inv_addequip" -or $Scenario -eq "wpn_relocate" -or $Scenario -eq "world_weapon_drop" -or $Scenario -like "world_item_*") { "1" } else { "" })
