...
function Test-PcAssault {
    param([string]$HostFile, [string]$JoinFile,
          [int]$MinFight = 3, [double]$MinHostDrop = 5.0,
    ...
    # Join-side drop for the SAME hurt hand (fall back to the picked victim).
    $joinDrop = $null; $joinFlesh = 0.0; $joinBlood = 0.0; $endGap = $null
    ...
    if ($fight -lt $MinFight) { $bad += "host-side fight never ran (hostview fight=1 x$fight, need >= $MinFight; link 4)" }
    if ($hostDrop -lt $MinHostDrop) { $bad += "no authoritative damage: host victim flesh/blood drop=$($m.hostDrop) (flesh=$($m.hostFlesh) blood=$($m.hostBlood); need >= $MinHostDrop; link 5 - THE join-does-no-damage bug)" }
    if ($null -eq $joinDrop -or $joinDrop -lt $MinJoinDrop) {
        if ($null -ne $endGap -and $endGap -le $MaxEndGap) {
            if ($hostDrop -ge $MinHostDrop) { $bad += "damage did not reach the join: join drop=$($m.joinDrop) while host dropped $($m.hostDrop) (link 6: vitals stream)" }
        } else {
            $bad += "damage did not reach the join: join drop=$($m.joinDrop) end-gap=$($m.endGap) (link 6: vitals stream)"
        }
    }
    # Combat cohesion FINDING (not a gate yet - calibrating thresholds from runs).
    ...
    if ($bad.Count -gt 0) {
        Write-Host ("  PC-ASSAULT FAIL - " + ($bad -join "; ") + " [spawn=$nSpawn cap=$capAll ord=$ordOk miss=$ordMiss fight=$fight hostDrop=$($m.hostDrop)(fl=$($m.hostFlesh)/bl=$($m.hostBlood)) joinDrop=$($m.joinDrop)]")
        return (Add-GateResult -Name "pc_assault" -Status FAIL -Metrics $m -Detail ($bad -join "; "))
    }
    Write-Host ("  PC-ASSAULT PASS - atk=$atk hurt=$hurtHand cap=$capAll host-orders=$ordOk hostview-fight=$fight; host flesh/blood drop=$($m.hostDrop) (flesh=$($m.hostFlesh)/blood=$($m.hostBlood)) streamed to join drop=$($m.joinDrop) (end-gap $($m.endGap))")
